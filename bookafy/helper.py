...
def generate_random_username(instance,first_name, length=15):
    # Generate a random string of uppercase letters and digits
    Klass = instance.__class__
    while True:
        username = ''.join(random.choices(first_name + string.ascii_uppercase + string.digits, k=length))
        # Check if the username is already taken
        if not Klass.objects.filter(username=username).exists():
            return username
