...
def get_data(user):
    return CustomNotification.objects.select_related('actor').filter(Q(type='like') | Q(type="comment"),recipient=user, unread=True)[:7]
...
class NotificationConsumer(AsyncJsonWebsocketConsumer):
    ...
    def fetch_notifications(self):
        ...
        notifications = CustomNotification.objects.select_related('actor').filter(Q(verb='like') | Q(verb="comment") ,recipient=user,
                                                                                  is_read=False)[:4]
